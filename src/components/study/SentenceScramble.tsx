...
export function SentenceScramble({ card, onSuccess }: SentenceScrambleProps) {
  ...
  if (!example) {
      return <div className="text-center p-4">No example sentence available for this card.</div>;
  }
  ...
  return (
    <div className="w-full max-w-2xl p-6 md:p-8 bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm border border-white/20 dark:border-white/10 rounded-2xl shadow-xl flex flex-col items-center gap-6 md:gap-8 relative overflow-hidden">
      ...
      <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Arrange the words to form the correct sentence.</p>
      </div>
      ...
      <div className={cn(
          ...
          status === "idle" ? "border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-950/30 hover:bg-slate-100 dark:hover:bg-slate-900/50 hover:border-indigo-400 dark:hover:border-indigo-500/50" : "",
          status === "correct" ? "border-green-500 bg-green-50/20 dark:bg-green-900/10" : "",
          status === "incorrect" ? "border-red-400 bg-red-50/20 dark:bg-red-900/10" : ""
      )}>
          {userAnswer.length === 0 && (
              <span className="text-slate-400 italic pointer-events-none select-none">Click words below to build sentence</span>
          )}
          {userAnswer.map((word) => (
              <Button
                key={word.id}
